# MSCE-Net Open-Source Package (HCE Module + nnU-Net v2 Integration)

This directory provides the HCE (Hierarchical Context Enhancement) module of MSCE-Net and its integration point in nnU-Net v2 for understanding and reuse.

## Included
- HCE module implementation
- nnU-Net v2 integration (applied to skip features)
...
- `nnunetv2/dynamic_network_architectures_local/architectures/unet.py`
  nnU-Net v2 integration: apply HCE to skip features in `PlainConvUNet`; the training/inference pipeline follows nnU-Net v2 conventions.
- `ckpt/`
  Training weights. Currently includes `checkpoint_best.pth`. For reference only; verify and adapt as needed.
...
nnU-Net v2 is an independent open-source project. This package contains integration code; please comply with the nnU-Net v2 license when redistributing derived files.
